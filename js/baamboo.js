...
const BaambooEngine = {
    ...
    renderGroupScores: function () {
        ...
        this.groups.forEach((g, index) => {
            ...
            box.style.cssText = 'cursor: pointer; background: rgba(255,255,255,0.05); border: 2px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 10px 20px; min-width: 100px; box-shadow: 0 8px 32px rgba(0,0,0,0.2); backdrop-filter: blur(10px); display: flex; flex-direction: column; align-items: center; transition: all 0.3s ease;';
            
            // Öğretmen manuel olarak aktif grubu değiştirebilsin
            box.onclick = () => {
    ...
    handleTimeout: function () {
        ...
        // Otomatik kapatma devredışı bırakıldı. Kapatmak için manuel olarak bbPassBtn kullanılmalıdır.
    ...
    selectOption: function (btn, selectedLetter, correctAnswer) {
        ...
        if (isCorrect) {
            ...
            // Otomatik kapatma devredışı bırakıldı.
            // setTimeout(() => this.closeQuestionModal(false), 3500);
        }
    },
