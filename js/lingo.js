...
const LingoEngine = {
    ...
    prepareFirebaseGame: function (config) {
        ...
        // Seçili set ID'leri varsa sadece onları çek, yoksa geniş tarama yap
        const selectedSetIds = config.SelectedSets
            ? config.SelectedSets.split(',').map(s => s.trim()).filter(s => s)
            : [];

        const fetchAndProcess = (setsPromise) => {
            setsPromise.then(snap => {
                let validWords = [];

                const processSet = (setKey, set) => {
                    // Eğer seçili set listesi varsa sadece onları dahil et
                    if (selectedSetIds.length > 0 && !selectedSetIds.includes(setKey)) return;

                    // Filter Preference (sadece geniş taramada uygula)
                    if (selectedSetIds.length === 0) {
                        ...
                        if (classGrades.length === 0 || classGrades.includes("Ortak") || classGrades.includes("Tümü")) classMatch = true;
                ...
                snap.forEach(child => processSet(child.key, child.val()));
        ...
        // Seçili setler varsa direkt tüm MasterPool'u oku (key bazlı filtre içeride yapılacak)
        // Yoksa type=wordspool indexiyle daha verimli sorgula
        if (selectedSetIds.length > 0) {
            fetchAndProcess(database.ref('MasterPool').once('value'));
        } else {
            fetchAndProcess(database.ref('MasterPool').orderByChild('Type').equalTo('wordspool').once('value'));
        }
    },
